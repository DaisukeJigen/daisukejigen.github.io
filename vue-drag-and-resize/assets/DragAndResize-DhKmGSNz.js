import { ap as defineComponent, aA as createElementBlock, aB as renderSlot, aC as normalizeClass, aD as useCssVars, aw as ref, ay as computed, aE as onMounted, aF as nextTick, aG as watch, aH as getCurrentInstance, as as openBlock } from "./vendor-D7sX2Tab.js";
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  emits: ["dragStart", "dragging", "dragEnd", "resizing", "resizeStart", "resizeEnd"],
  setup(__props, { expose: __expose, emit: __emit }) {
    ...
    const borders = ref({ bottom: 0, left: 0, right: 0, top: 0 }), boundries = ref(), clickedAtPosition = ref({ x: 0, y: 0 }), collideBottom = computed(
      ...
      if (collideLeft.value) {
        divPos.value.x = ((_a = boundries.value) == null ? void 0 : _a.x.min) ?? 0;
      }
      if (collideRight.value) {
        divPos.value.x = (((_b = boundries.value) == null ? void 0 : _b.x.max) ?? 0) - size.value.x;
      }
      if (collideTop.value) {
        divPos.value.y = ((_c = boundries.value) == null ? void 0 : _c.y.min) ?? 0;
      }
      if (collideBottom.value) {
        divPos.value.y = (((_d = boundries.value) == null ? void 0 : _d.y.max) ?? 0) - size.value.y;
      }
    }, corners = computed(() => {
      return {
        bottomLeft: {
    ...
    }), cssHeight = computed(() => size.value.y + "px"), cssLeft = computed(() => divPos.value.x + "px"), cssPosition = computed(() => props.position), cssTop = computed(() => divPos.value.y + "px"), cssWidth = computed(() => size.value.x == -1 ? "auto" : size.value.x + "px"), divPos = ref({ x: 0, y: 0 }), divPosAtDragStart = ref({ x: 0, y: 0 }), divPosWhenClicked = ref({ x: 0, y: 0 }), drag = () => {
      emit("dragging");
      divPos.value = {
      ...
      collisionCheck();
    }, emit = __emit, getParent = () => {
      var _a, _b, _c;
      if ((_a = thisDiv.value) == null ? void 0 : _a.parentNode.classList.contains("vueDragAndResize")) {
        return (_b = thisDiv.value) == null ? void 0 : _b.parentNode;
      }
      return props.position == "relative" ? (_c = thisDiv.value) == null ? void 0 : _c.parentNode : document.documentElement;
    }, inRange = (n, s, e) => {
      return n > s && n < e;
    }, mousePosition = ref({ x: 0, y: 0 }), mousePositionInParent = ref({ x: 0, y: 0 }), movedX = computed(() => mousePosition.value.x - clickedAtPosition.value.x), movedY = computed(() => mousePosition.value.y - clickedAtPosition.value.y), parentBorders = ref({ bottom: 0, left: 0, right: 0, top: 0 }), parentPosition = ref({ x: 0, y: 0 }), parentSize = ref({ x: 0, y: 0 }), props = __props, resize = () => {
      emit("resizing");
      ...
      var _a, _b, _c, _d;
      if (inRange(mousePositionInParent.value.y, ((_a = boundries.value) == null ? void 0 : _a.y.min) ?? 0, ((_b = boundries.value) == null ? void 0 : _b.y.max) ?? 0)) {
        size.value.y += movedY.value;
      }
      if (mousePositionInParent.value.y > (((_c = boundries.value) == null ? void 0 : _c.y.max) ?? 0)) {
        size.value.y = (((_d = boundries.value) == null ? void 0 : _d.y.max) ?? 0) - divPos.value.y;
      }
      ...
      if (mousePositionInParent.value.x < (((_a = boundries.value) == null ? void 0 : _a.x.min) ?? 0)) {
      ...
      var _a, _b, _c, _d;
      if (inRange(mousePositionInParent.value.x, ((_a = boundries.value) == null ? void 0 : _a.x.min) ?? 0, ((_b = boundries.value) == null ? void 0 : _b.x.max) ?? 0)) {
        size.value.x += movedX.value;
      }
      if (mousePositionInParent.value.x > (((_c = boundries.value) == null ? void 0 : _c.x.max) ?? 0)) {
        size.value.x = (((_d = boundries.value) == null ? void 0 : _d.x.max) ?? 0) - divPos.value.x;
      }
      clickedAtPosition.value.x = mousePosition.value.x;
    }, resizedUp = () => {
      var _a, _b, _c;
      ...
      if (mousePositionInParent.value.y < (((_c = boundries.value) == null ? void 0 : _c.y.min) ?? 0)) {
    ...
    }, size = ref({ x: 0, y: 0 }), sizeAtResizeStart = ref({ x: 0, y: 0 }), thisDiv = computed(() => thisInstance == null ? void 0 : thisInstance.vnode.el), thisInstance = getCurrentInstance(), zIndexAdjust = () => {
      const dar = document.querySelectorAll(`.vueDragAndResize[data-aot="1"]`);
      if (dar.length > 1) {
        const divs = [];
        let z = 99999;
        dar.forEach((element) => {
          if (element.getAttribute("data-uid") == (thisInstance == null ? void 0 : thisInstance.uid)) {
        ...
        divs.forEach((d) => {
          document.querySelectorAll(`[data-uid='${d.uid}']`)[0].style.zIndex = (--z).toString();
        });
      }
    };
    __expose({ boundries, corners });
    onMounted(() => {
      var _a, _b, _c, _d, _e;
      ...
      }).observe((_b = thisDiv.value) == null ? void 0 : _b.parentNode, {
        attributes: true,
        // Watch for attribute changes
        childList: false,
        // Watch for changes to the children
        subtree: false
        // Watch for changes in all descendant nodes
      });
      (_c = thisDiv.value) == null ? void 0 : _c.addEventListener("mousedown", (event) => {
      ...
      const rect = (_d = thisDiv.value) == null ? void 0 : _d.getBoundingClientRect();
      divPos.value = {
      ...
      if (props.dragOn != "") {
        const dragItem = props.dragOn == "" ? thisDiv.value : (_e = thisDiv.value) == null ? void 0 : _e.querySelectorAll(props.dragOn);
        dragItem.forEach((di) => {
    ...
    const __returned__ = { borders, boundries, clickedAtPosition, collideBottom, collideLeft, collideRight, collideTop, collisionCheck, corners, cssHeight, cssLeft, cssPosition, cssTop, cssWidth, divPos, divPosAtDragStart, divPosWhenClicked, drag, emit, getParent, inRange, mousePosition, mousePositionInParent, movedX, movedY, parentBorders, parentPosition, parentSize, props, resize, resizedDown, resizedLeft, resizedRight, resizedUp, resizeMode, resizeModeWhenClicked, selected, setBoundries, setParentInfo, size, sizeAtResizeStart, thisDiv, thisInstance, zIndexAdjust };
    Object.defineProperty(__returned__, "__isScriptSetup", { enumerable: false, value: true });
    return __returned__;
  }
});
const _hoisted_1 = ["data-uid", "data-aot"];
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  ...
  return openBlock(), createElementBlock("div", {
    ...
    "data-uid": (_a = $setup.thisInstance) == null ? void 0 : _a.uid,
    "data-aot": $setup.props.alwaysOnTop ? 1 : 0
  }, [
